...
TEST(KeyLengths, KeyLength128) {
  ...
  EXPECT_FALSE(memcmp(expected, out, BLOCK_BYTES_LENGTH));
  EXPECT_EQ(BLOCK_BYTES_LENGTH, len);
  delete[] out;
}
...
TEST(ECB, OneBlockWithoutByteEncrypt) {
  ...
  EXPECT_EQ(BLOCK_BYTES_LENGTH, len);
  EXPECT_FALSE(memcmp(expected, out, len));
...
TEST(CBC, TwoBlocksDecrypt) {
  AES aes(128);
  unsigned char encrypted[] =
      {0x1b, 0x87, 0x23, 0x78, 0x79, 0x5f, 0x4f, 0xfd, 0x77, 0x28, 0x55, 0xfc, 0x87, 0xca, 0x96, 0x4d,
